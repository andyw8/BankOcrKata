  ...
  describe CheckSum do
    ...
    describe "#check?" do
      context "when the account number is good" do
        ...
        Then { checker.check?("000000000").should be(true) }
        Then { checker.check?("000000051").should be(true) }
        Then { checker.check?("123456789").should be(true) }
        Then { checker.check?("200800000").should be(true) }
        Then { checker.check?("333393333").should be(true) }
        Then { checker.check?("490867715").should be(true) }
        Then { checker.check?("664371485").should be(true) }
        Then { checker.check?("711111111").should be(true) }
        Then { checker.check?("777777177").should be(true) }
      end

      context "when the account number is bad" do
        # bad account numbers were taken from the user story specs
        Then { checker.check?("111111111").should_not be(true) }
        Then { checker.check?("490067715").should_not be(true) }
        Then { checker.check?("664371495").should_not be(true) }
        Then { checker.check?("00000000").should_not be(true) }
        Then { checker.check?("0000000000").should_not be(true) }
      end
    end
  end
